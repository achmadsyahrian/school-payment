...
<div class="section">
   ...
   <div class="section-header">
      <div class="section-header-back">
         <a href="{{ route('employee.payments.studentDetail', [$student->nisn]) }}" class="btn btn-icon"><i class="fas fa-arrow-left"></i></a>
      </div>
      <h1>Pembayaran Periode {{$academicYearDetail->name}}</h1>
      <div class="section-header-breadcrumb">
         <div class="breadcrumb-item active"><a href="/">Dashboard</a></div>
         <div class="breadcrumb-item active"><a href="{{ route('employee.payments.index') }}">Pembayaran</a></div>
                  ...
                  <div class="col-12 col-md-6 col-lg-3">
                     <div class="card card {{ $isPaid ? 'card-success' : 'card-primary' }}">
                        ...
                        <div class="card-body">
                           ...
                           @else
                              <form class="d-inline" 
                                    action="{{ route('employee.payments.storePayment', [$student->id, $academicYearDetail->id, $index+1]) }}" 
                                    method="post" id="payment-store-{{ $index }}">
                                    @csrf
                                    <button type="button" class="btn btn-primary btn-action"
                                          onclick="showDeleteConfirmation('Ya, Bayar', 'Apakah anda yakin ingin melunasi bulan ini?', 'payment-store-{{ $index }}')">
